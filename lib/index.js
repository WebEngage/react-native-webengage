...
AppRegistry.registerHeadlessTask('WebEngageHeadless', (remoteMessage) =>{
	console.log("WebEngageBridge: HeadlessJS called! ",pushReceivedHandler)
	if(pushReceivedHandler) {
		return remoteMessage => pushReceivedHandler(remoteMessage);
	}
...
function WebEngagePushChannel () {
	...
	this.onPushReceivedCallback = function() {};
	this._options = {};
	this.pushClickListener = addListenerToBridge('pushNotificationClicked', (data) => this.clickCallback(data));
	this.pushReceivedListener = addListenerToBridge('pushNotificationReceived', (data) => this.onPushReceivedCallback(data));
}
...
WebEngagePushChannel.prototype.onPushReceived = function (callback) {
	WebEngage.updateListenerCount()
	this.onPushReceivedCallback = callback; // For foreground
	pushReceivedHandler = callback; // Background and Killed
	return this.pushReceivedListener;
};
	...
	if (type) {
		switch(type) {
			case 'shown':
				break;
			case 'received':
				this.onPushReceivedCallback(customData)
				break;
