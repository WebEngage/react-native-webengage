...
function WebEngagePushChannel () {
	...
	this._options = {};
	addListenerToBridge('pushNotificationClicked', (data) => this.clickCallback(data));
}
...
function WebEngageUniversalLinkChannel () {
	this.clickCallback = function(){};
	this._options = {};
	addListenerToBridge('universalLinkClicked', (location) => this.clickCallback(location));
}
...
function WebEngageNotificationChannel () {
	...
	addListenerToBridge('notificationClicked', (data) => this.clickCallback(data, data["clickId"]));

	addListenerToBridge('notificationDismissed', (data) => this.dismissCallback(data));
	addListenerToBridge('notificationPrepared', (data) => this.prepareCallback(data));
	addListenerToBridge('notificationShown', (data) => this.shownCallback(data));
}

function addListenerToBridge(method, callback) {
	console.log("addListenerToBridge " + method)
	const webengage = new NativeEventEmitter(NativeModules.webengageBridge);
	WebEngage.updateListenerCount()
	console.log("updateListenerCount after " + method)
	webengage.addListener(method, callback)
}
