...
WebEngageUserChannel.prototype.login = function(userId, jwtToken = null) {
	if(jwtToken) {
		WebEngage.loginWithJwt(userId, jwtToken);
	} else {
		WebEngage.login(userId);
	}
};

WebEngageUserChannel.prototype.invalidateTokenCallback = function(callback) {
...
WebEngageUserChannel.prototype.setSecurityToken = function(cuid, secureToken) {
	WebEngage.setSecurityToken(cuid, secureToken);
};
