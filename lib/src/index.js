...
WebEngagePlugin.prototype.init = function(autoRegister) {
	WebEngageModule.init(autoRegister);
};
...
WebEngageUserChannel.prototype.setAttribute = function(key, value) {
	if (Platform.OS === 'ios') {
		WebEngageModule.setAttribute(key, value);
	} else {
		var map = new Object(); // or var map = {};
		map[key] = value;
		WebEngageModule.setAttribute(map);
	}
};
...
function addListenerToBridge(method, callback) {
	if(Platform.OS === 'android') {
		...
		WebEngageModule.onEventListenerAdded && WebEngageModule.onEventListenerAdded(method);
		return listener
	}
	if(Platform.OS === 'ios') {
		const webengage = new NativeEventEmitter(WebEngageModule);
...
WebEngageNotificationChannel.prototype.onPrepare = function (callback) {
	console.log("test: onPrepare called | calling updateListenerCount");
	WebEngageModule.updateListenerCount()
